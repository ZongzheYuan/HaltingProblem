\chapter{Definition in the Universal \WHILE model}
...
\end{code}
Then, there are sixteen constant in \md{D} that are used to indicate special meaning in the universal \WHILE program.
...
do=? = const 16
\end{code}
\section{Code \WHILE}\label{appendix:code while}
...
\section{Variable of Universal \WHILE Program}\label{appendix:variable of u}
...
write Vl;
\end{code}
Where the \wit{STEP} is the Macro that simulate the program.
\section{STEP Macro}\label{appendix:step}
When we are simulate the \WHILE program which has only one variable, we can easily define the stack machine as a Macro based on three variables: \wit{Cd} which is the \wit{command}, \wit{St} which is the stack and \wit{V1} which is the only one variable.
...
\section{Constant in Expression}\label{appendix:const of e}
First of all, we define a function that would construct some value in the format of \md{E} based on the the data of \md{D}:
\begin{code}
dtoE : {n : ℕ} → D → E n
dtoE dnil = nil
